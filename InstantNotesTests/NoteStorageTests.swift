...
struct NoteStorageTests {
    ...
    @Test("The models meet CloudKit's rules, so notes can sync")
    func cloudKitSchema() throws {
        let url = FileManager.default.temporaryDirectory.appending(path: "\(UUID()).store")
        defer { try? FileManager.default.removeItem(at: url) }
        // Loading a CloudKit-backed store checks the schema first and throws on anything CloudKit
        // can't hold: unique attributes, required values without defaults, missing inverses.
        let config = ModelConfiguration(schema: container.schema, url: url,
                                        cloudKitDatabase: .private("iCloud.com.homezero1.instantnotes"))
        _ = try ModelContainer(for: container.schema, configurations: [config])
    }
